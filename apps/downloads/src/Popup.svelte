<script lang="ts">
	...
	function updateDownloads() {
		...
		chrome.storage.local.get().then((dlStates) => {
			states = dlStates

			// poll for updates if any download is currently downloading
			if (Object.values(states).includes(DownloadState.downloading)) setTimeout(() => updateDownloads(), 500)
		})
	}
